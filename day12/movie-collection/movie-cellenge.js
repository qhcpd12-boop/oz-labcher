const targetGenre = "드라마";//찾고싶은 드라마 장르

const prepareMovies = () => {//영화데이터를 만드는 함수
  const movies = [];//영화들을 저장할 배열이[]이다
  //영화 객체만들기
  let movie1 = {
    title: "쇼생크 탈출",
    director: "프랭크 다라본트",
...
//영화 배열들에 무비3,1,2를 추가한다. 이때 무지앞 숫자는 상관 없고 배열의 순서대로 출력된다
  movies.push(movie3);
  movies.push(movie1);
  movies.push(movie2);
  //movies.push배열에 객체를 직접넣어도 화면에같게 출력 
  movies.push({
    title: "어벤져스: 엔드게임",
  ...
  return movies;//만든영화배열을 밖으로 반환해 출력
};
//장르에 따른 영화를 필터해 찾는다((영화들의,장르)
